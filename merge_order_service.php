...
<?php
    //调用
        $ys = new YSPay();
        $merchantId=$ys->getMerchantId();
        $url=$ys->getPostUrl();
        $ys->SetInfo("subject", "测试订单2");
        $ys->SetInfo("body", "easypay.merchant.merge.pay");
        $ys->SetInfo("merchant_id", "$merchantId");
        $ys->SetInfo("out_merge_no", "M202002281582892809416");
        ...
        $arr = array(
            ...
            array(
                ...
                "out_trade_no" => "SUB1202002281582892809416",
                "total_amount" => "1",
                "seller_email" => "sellerEmail",
                "buyer_email" => "sellbuyer_emailerEmail"
            )
        );
        $ys->SetInfo("order_details", $arr);
        $service="easypay.merchant.merge.pay";
    
        $biz_content=json_encode($ys->GetAll(), JSON_UNESCAPED_UNICODE);
        $sign=$ys->GetSign();
        ...
        echo "			<input type='hidden' name='sign' id='sign' value='$sign' />";
        echo "			 <input type='hidden' name='charset' id='charset' value='UTF-8' />";
        echo "			 <input type='hidden' name='biz_content' id='biz_content' value='$biz_content' />";
        echo "			 <input type='hidden' name='partner' id='partner' value='$merchantId' />";
        echo "			 <input type='hidden' name='sign_type' id='sign_type' value='RSA' />";
        echo "			 <input type='hidden' name='service' id='service' value='$service' />";
        echo "		</form>";
        echo "	</body>";
        echo "	<script type=\"text/javascript\">";
        echo "		document.all.pay_form.submit();";
        echo "	</script>";
        echo "</html>";
?>
